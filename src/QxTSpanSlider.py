#Original code from https://github.com/mkilling/QxtSpanSlider.py
#Ported to pyQt5

from PyQt5.QtCore import *
from PyQt5.QtGui import QPen, QPalette, QLinearGradient, QColor
...
class QxtSpanSlider(QSlider):
    ...
    spanChangedSig = pyqtSignal(int, int, name='spanChanged')
    lowerValueChangedSig = pyqtSignal(int, name='lowerValueChanged')
    upperValueChangedSig = pyqtSignal(int, name='upperValueChanged')
    lowerPositionChangedSig = pyqtSignal(int, name='lowerPositionChanged')
    upperPositionChangedSig = pyqtSignal(int, name='upperPositionChanged')
    sliderPressedSig = pyqtSignal(int, name='sliderPressed')

    rangeChangedSig = pyqtSignal(int,int,name='rangeChanged')
    sliderReleasedSig = pyqtSignal(name='sliderReleased')
    ...
    def __init__(self, parent = None):
        ...
        self.rangeChangedSig.connect(self.updateRange)
        self.sliderReleasedSig.connect(self.movePressedHandle)
    ...
    def setSpan(self, lower, upper):
        ...
        if upp != self.upper:
            ...
            changed = True
        if changed:
            self.spanChangedSig.emit(self.lower, self.upper)
    ...
    def setLowerPosition(self, lower):
        if self.lowerPos != lower:
            ...
            if self.isSliderDown():
                self.lowerPositionChangedSig.emit(lower)
    ...
    def setUpperPosition(self, upper):
        if self.upperPos != upper:
            ...
            if not self.hasTracking():
                self.update()
            if self.isSliderDown():
                self.upperPositionChangedSig.emit(upper)
    ...
    def handleMousePress(self, pos, control, value, handle):
        ...
        if control == QStyle.SC_SliderHandle:
            ...
            self.sliderPressedSig.emit(handle)
    ...
    handleMovementMode = pyqtProperty("int", handleMovementMode, setHandleMovementMode)
    gradientLeftColor = pyqtProperty("QColor", gradientLeftColor, setGradientLeftColor)
    gradientRightColor = pyqtProperty("QColor", gradientRightColor, setGradientRightColor)
